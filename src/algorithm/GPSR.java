...
{	r_list=new ArrayList();
	...
	for(int ii=0;ii>=0;ii++)
		{	
			...
			while(it.hasNext())
				{	
					ArrayList a_now=(ArrayList)it.next(); //node i's routing
					//judge if node i is the head
					System.out.println("now adress the node "+(int)a.get(count));
		...
		float w_clt=em.get_2D(s[id_clt].position_x, s[id_clt].position_y, s[head].position_x, s[head].position_y);
		
		for(int j=0;j<id.size();j++)
			{
				if(em.get_2D(s[(int)id.get(j)].position_x, s[(int)id.get(j)].position_y, s[head].position_x, s[head].position_y)<w_clt)
					{
						w_clt=(float)ws.get(j);
						if(w_clt==0) {System.out.println("node "+a.get(count)+" can get to head");}
						id_clt=(int)id.get(j);
					}
			}
		System.out.println(a.get(count)+": the node closest to head is "+id_clt);
